...
void HelpTreeBase::AddMyVertexInfo(  unsigned int number, const std::string vertexName ) {
  ...
  // Add these basic branches
    ...
    std::string sumtrkptbranchname = vertexName+"SumTrkPt";
    ...
    m_tree->Branch( sumtrkptbranchname.c_str(), &m_vertex_sumtrkpt[sumtrkptbranchname] );


}

void HelpTreeBase::FillMyVertexInfo( std::vector<float> vertex_sumptsquared, std::string vertexName  ) {
  ...
  //m_vertexsumpt = vertex_sumptsquared;
  int pv0pos = std::distance(vertex_sumptsquared.begin(), std::max_element(vertex_sumptsquared.begin(), vertex_sumptsquared.end()));
  int npv = vertex_sumptsquared.size();
  ...
  m_vertex_sumtrkpt[sumtrkptbranchname] = vertex_sumptsquared;
